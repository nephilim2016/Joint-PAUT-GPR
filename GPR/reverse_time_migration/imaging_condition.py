...
def imaging_condition_correlation_poynting(sigma,epsilon,index,CPML_Params):
    #Get Forward Params
    f=WaveletType(config.t, config.frequency, config.wavelet_type).create_wavelet()
    #Get Forward Data ----> <Generator>
    forward_generator=update_field_forward.time_loop(config.xl,config.zl,config.dx,config.dz,config.dt,
...
def imaging_condition(iepsilon): 
    ...
    elif config.imaging_condition=='poynting':
        imaging_result=0
        source_result=0
        for index,value in enumerate(config.receiver_site):
            res=pool.apply_async(imaging_condition_correlation_poynting,args=(sigma.copy(),iepsilon.copy(),index,CPML_Params))
            res_l.append(res)
        pool.close()
        pool.join()
