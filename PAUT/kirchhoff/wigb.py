...
def insert_zeros(trace, tt=None):
    ...
    # insert zeros in tt and trace#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sun Jan 27 17:03:48 2019
...
def wiggle(data, tt=None, xx=None, color='k', star=0, sf=0.15, verbose=False):
    '''Wiggle plot of a sesimic data section
    Syntax examples:
        wiggle(data)
        wiggle(data, tt)
    ...
    for ntr in range(Ntr):
        ...
        if ntr==star:
            ax.fill_betweenx(tt_zi, offset, trace_zi + offset,
                             where=trace_zi >= 0,
                             facecolor='r')
            ax.plot(trace_zi + offset, tt_zi, 'r')
        else:
            ax.fill_betweenx(tt_zi, offset, trace_zi + offset,
                             where=trace_zi >= 0,
                             facecolor=color)
...
if __name__=='__main__':
    ...
    data=np.load('./%sHz_forward_data_file/%s_record.npy'%(5e4,idx))
    # data[:,10]=np.mean(data[:,9]+data[:,11])
    # wiggle(Data_,xx=np.arange(200),color='k',sf=0.2,verbose=False)
    
    pyplot.figure()
    Data_=WiggleDataNormalized.WiggleDataNormalized_(data)
    wiggle(Data_,xx=np.arange(20),color='k',sf=1,verbose=False)
    pyplot.savefig('%s.png'%idx,dpi=1000)
    # ax=pyplot.gca()
    # ax.set_yticks(np.linspace(0,1500,7))
    # ax.set_yticklabels([0,10,20,30,40,50,60])
    # ax.set_xlabel('Trace')
    # ax.set_ylabel('Time (ns)')
    # # pyplot.savefig('FieldTestData.png',dpi=1000)
    
    # pyplot.figure()
    # Data_=WiggleDataNormalized.WiggleDataNormalized_(PredictData)
    # wiggle(Data_,xx=np.arange(194),color='k',sf=1,verbose=False)
    # ax=pyplot.gca()
    # ax.set_yticks(np.linspace(0,512,7))
    # ax.set_yticklabels([0,10,20,30,40,50,60])
    # ax.set_xlabel('Trace')
    # ax.set_ylabel('Time (ns)')
    # # pyplot.savefig('FieldPredictData.png',dpi=1000)
