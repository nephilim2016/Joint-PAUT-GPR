...
#Add CPML condition
class Add_CPML():
    def __init__(self,xl,zl,rho,vp,dx,dz,dt):
        self.npml=10
        self.Npower=4
        self.k_max_CPML=5
        ...
        self.add_CPML(rho,vp,dx,dz,dt)    
    
    def add_CPML(self,rho,vp,dx,dz,dt):
        ...
        vp_max=np.max(vp)
        sig0_x=-(self.Npower+1)*vp_max*np.log(1e-8)/(2*thickness_CPML_x)
        sig0_z=-(self.Npower+1)*vp_max*np.log(1e-8)/(2*thickness_CPML_z)
