...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 27 07:28:01 2025
...
if __name__ == '__main__':
    ...
    for idx in range(15, 8, -1):
        raw = np.load(f'./{idx}_poynting_rtm_result.npy')
        # raw = np.load(f'./{idx}_correlation_rtm_result.npy')
        raw = raw[10:-10,10:-10]
    ...
    plt.ylabel('Depth (m)')
    # plt.savefig('gather_poynting_rtm.png',dpi=1000)
    # plt.savefig('gather_correlation_rtm.png',dpi=1000)
    ...
    plt.xlabel('Distance (m)')
    plt.ylabel('Depth (m)')
    # plt.savefig('gather_poynting_env.png',dpi=1000)
    # plt.savefig('gather_correlation_env.png',dpi=1000)
    
    
    np.save('env_u.npy',env)